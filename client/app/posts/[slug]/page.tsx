...
export default async function PostPage({ params }: PageProps) {
  ...
  const handleCheckIdExisted = (ids: string[], id: string): string => {
    const idExisted = ids.includes(id);
    if (idExisted) {
      let counter = 0;
      do {
        counter += 1;
  ...
  const extractHeadings = (content: string) => {
    ...
    while ((match = headingRegex.exec(content)) !== null) {
      ...
      const idExisted = headings.find((heading) => heading.id === id);
      if (idExisted) {
        let counter = 0;
        do {
          counter += 1;
          id = `${id}-${counter}`;
        } while (headings.some((heading) => heading.id === `${id}-${counter}`));
      }
          ...
          <div className="text-center text-muted-foreground">
            ...
            <p className="text-sm mt-2">
              ...
              {post.updated_at !== post.created_at && (
                <span>
                  {' '}
                  • Last updated on {formatDatetime(post.updated_at)}
                </span>
              )}
            </p>
          </div>
